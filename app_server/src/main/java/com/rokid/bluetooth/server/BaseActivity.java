...
public abstract class BaseActivity extends Activity {
    ...
    public void onChooseImage(String path) {
        ...
        try {
            ...
            Log.d("Rokid_Bt", "[Service]onChooseImage 开始发送，size="+msg.transferMessage.dataList.size()+
                    ", bytes="+msg.transferMessage.dataList.get(0).bytes.length+
                    ", name="+msg.transferMessage.dataList.get(0).name+
                    ", crc="+msg.transferMessage.dataList.get(0).crc);
        } catch (Exception e) {
            e.printStackTrace();
        }
